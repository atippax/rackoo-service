export function createIdTeam(memberLength: number) {
  const members = [];
...
export function createTeamHeaderAndBody(members, limitTeam = 2) {
  const header = [];
  const body = [];
  let key = 0;
...
export async function createUniquePair({ header, body }, estimate, limitTeam) {
...
export async function createPairTemplate(memberLength, limit, estimate = 4) {
...
export async function createTeamPair(
  member,
  limitTeam,
  limit,
  start,
